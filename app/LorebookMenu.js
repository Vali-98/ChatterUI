    ...
    return (
        ...
        <Stack.Screen options={{
            ...
            <View style={styles.bar}>
                ...
                <TouchableOpacity  style={styles.button} onPress={() => {setShowNewBook(true)}}>
                        <FontAwesome size={24} name='plus' color={Color.Button} />
                </TouchableOpacity>
            </View>
            <ScrollView style={{marginTop: 16}} >
                {books.length > 0 && books.filter((book) => {return book.toLowerCase().includes(searchData.toLowerCase())}).map((book, index) => (
                    <TouchableOpacity  key={index} style={styles.lorebookItem} onPress={() => router.push(`/LorebookInfo/${book}`)}>
                        <Text style={{color: Color.Text}}>{book}</Text>
                        <TouchableOpacity>
                            <FontAwesome color={Color.White} size={24} name='trash' onPress={() => Lorebooks.deleteFile(book).then(() => loadBooksList())} />
                        </TouchableOpacity>
                    </TouchableOpacity>
                ))}
            </ScrollView>
