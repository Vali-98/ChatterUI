...
export namespace Llama {
    ...
    export const loadModel = async (name : string, preset = default_preset, usecache = true) => {
        ...
        await FS.getInfoAsync(`${model_dir}session.bin`).then(result => {
            ...
            if(llamaContext)
            llamaContext.loadSession(result.uri)
        })
    }
      
    export const completion = async (prompt : CompletionParams, callback = (text : string) => {console.log(text)}) => {
    ...
    export const downloadModel = async (url : string) => {
        ...
        await FS.downloadAsync(url, `${model_dir}${modelName}`).then(() => {
    ...
    export const importModel = async () => {
        return DocumentPicker.pickSingle().then(async (result : any) => {
    ...
    export const getModelname = () => {return modelname}

    export const setLlamaPreset = () => {
        const presets = mmkv.getString(Global.LocalPreset)
        if(presets == undefined)
            mmkv.set(Global.LocalPreset, JSON.stringify(default_preset))
    }
}
