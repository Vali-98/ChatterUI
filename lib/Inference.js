...
const constructTGWUIPayload = (messages) => {
    ...
    const context_len = (APIType === API.MANCER)? Math.min(preset.max_length , mancerModel.limits.context) : preset.max_length
    const gen_len = (APIType === API.MANCER)? Math.min(preset.genamt , mancerModel.limits.completion) : preset.genamt
    return {
        stream : true,
        ...
        typical_p: parseFloat(preset.typical_p),
...
const TGWUIReponseStream = async (setAbortFunction, insertGeneratedMessage, messages) => {
    ...
    const tgwuiendpoint = getString(Global.TGWUIStreamingEndpoint)

    readableStreamResponse( 
        `${tgwuiendpoint}/v1/completions`,
...
const readableStreamResponse = (endpoint , payload, insertGeneratedMessage, jsonreader, setAbortFunction, abort_func = () => {}) => {
