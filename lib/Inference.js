...
const buildContext = (max_length, messages) => {
    ...
    let payload = `${currentInstruct.system_sequence_prefix}
    ...
    for (const message of messages.slice(1).reverse()) {
        const message_shard = `${message.name === charName ? currentInstruct.output_sequence : currentInstruct.input_sequence}
        ${currentInstruct.names ? message.name + ': ' : ''}
        ${message.mes}
        ${currentInstruct.wrap ? '\n' : ' '}`
...
const KAIresponse = async (setAbortFunction, insertGeneratedMessage, messages) => {
    ...
    readableStreamResponse(
        ...
        (item) => {
            return JSON.parse(item.substring(5)).token
        },
        setAbortFunction,
        () => {
            axios
                ...
                .catch(() => {
                    ToastAndroid.show(`Abort Failed`, 2000)
                })
        }
    )
}
...
const readableStreamResponse = async (
    ...
    fetch(endpoint, {
        ...
        .catch((error) => {
            stopGenerating()
            if (!aborted) ToastAndroid.show('Connection Lost...', ToastAndroid.SHORT)
