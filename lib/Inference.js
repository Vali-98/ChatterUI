...
export const generateResponse = async (setAbortFunction, insertGeneratedMessage, messages) => {
...
const buildContext = (max_length, messages) => {
    ...
    let message_acc = ``
    let payload_length = llamaTokenizer.encode(payload).length
    ...
    for(const message of messages.slice(1).reverse()) {
        ...
        let shard_length = llamaTokenizer.encode(message_shard).length
...
const buildContextAsync = (max_length, messages) => {
    return new Promise((resolve, reject) => {
        try {
            let result = buildContext(max_length, messages);
            resolve(result);
        } catch (error) {
            reject(error);
        }
...
const instructReplaceMacro = () => {
    ...
    console.log(instruct.input_sequence)
...
const readableStreamResponse = async (endpoint , payload, insertGeneratedMessage, jsonreader, setAbortFunction, abort_func = () => {}, header = {}) => {
    ...
    .then(async (response) => {
        ...
        return reader.read().then(function processText ({done, value}) {
            ...
            for(e of events)
            {
                ...
                if(datasum != '')
                insertGeneratedMessage(datasum)
            }
...
const localStreamResponse = async (setAbortFunction, insertGeneratedMessage, messages) => {
    ...
    Llama.completion(await constructLocalPayload(messages), insertGeneratedMessage).then(() => {
        setValue(Global.NowGenerating, false)
    }).catch((error) => {
        console.log(`Failed to generate locally: `, error)
        ToastAndroid.show(`Failed to generate locally: ${error}`, 2000)
        setValue(Global.NowGenerating, false)
    })
