...
export const generateResponse = (setAbortFunction, insertGeneratedMessage, messages) => {
    ...
    const adventureMode = getBool(Global.AdventureEnabled)
    console.log(adventureMode)
    try {
        if(adventureMode)
        ...
        switch(APIType) {
            case API.KAI:
                KAIresponseStream(setAbortFunction, insertGeneratedMessage, messages)
                break
            case API.HORDE:
...
const adventure_grammar = () => {

    let emotions = ["SAD" , "HAPPY" , "NEUTRAL" , "ANGRY" , "CONFUSED"]
    emotions.sort()

    return `text ::= [^\\n]+ "\\n"
    action ::= [^\\n^\".\"]+ ".\\n"
    root ::= "\`\`\`\\n{{Actions}}:\\n" "[${emotions[0]}] "  action  "[${emotions[1]}] "   action "[${emotions[2]}] "  action `
} 
...
const constructKAIPayload = (messages) => {
    ...
    return {
        ...
        "grammar" : (adventureMode && messages.at(-1).name === charName && messages.at(-1).adventure_options === '' ) ? adventure_grammar():  presetKAI.grammar
    }
}
...
const KAIresponseStream = (setAbortFunction, insertGeneratedMessage, messages) => {
...
const KAIresponse  = async (setAbortFunction, insertGeneratedMessage, messages) => {
   ...
   // const timeout = setTimeout(() => controller.abort(), 60000);
    ...
    fetch(`${kaiendpoint}/api/v1/generate`, {
        reactNative: {textStreaming: true},
        method: `POST`,
        body: JSON.stringify(constructKAIPayload(messages)),
        signal: controller.signal,
        ...
        setValue(Global.NowGenerating, false)
        insertGeneratedMessage(response.results[0].text ?? '')
    }).catch((error) => {
        setValue(Global.NowGenerating, false)
        if(!aborted)
            ToastAndroid.show('Connection Lost...', ToastAndroid.SHORT)
