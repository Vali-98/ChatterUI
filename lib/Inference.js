...
import { mmkv, Global, API, hordeHeader, Llama, Logger, Messages } from '@globals'
...
const readableStreamResponse = async (
    ...
    Messages.useBuffer.setState({ buffer: messages.at(-1).mes })
    ...
    es.addEventListener('message', (event) => {
        ...
        const buffer = Messages.useBuffer.getState().buffer
        const cleaned = (buffer + text)
            ...
            .replaceAll(userName + ':', '')
            .replaceAll(charName + ':', '')
        Messages.useBuffer.getState().setBuffer(cleaned)
    })
