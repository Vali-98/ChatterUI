...
const buildChatCompletionContext = async ({
    apiConfig,
    apiValues,
    messages,
    character,
    ...
    let initial = `${instruct.system_prompt}
    \n${user?.description ?? ''}
    ...
    if (instruct.scenario && character?.scenario) {
        initial += character.scenario
        total_length += characterCache.scenario_length
    }

    if (instruct.scenario && character?.personality) {
    ...
    const payload: Message[] = [
        {
            ...
            [completionFeats.contentName]: replaceMacrosInternal(initial),
        },
    ]
    let hasImage = false
        ...
        if (message.attachments.length > 0) {
            ...
            messageBuffer.push({
                ...
                [completionFeats.contentName]: [
                    {
                        type: 'text',
                        text: replaceMacrosInternal(prefill + swipe_data.swipe),
                    },
                    ...images,
                ],
            })
            ...
            messageBuffer.push({
                role: role,
                [completionFeats.contentName]: replaceMacrosInternal(prefill + swipe_data.swipe),
            })
        }
...
export const buildTextCompletionContext = async ({
    ...
    const delta = performance.now()
    const userCardData = (user?.description ?? '').trim()
    const charCardData = (character?.description ?? '').trim()
    ...
    if (instruct.system_prefix) {
        payload += instruct.system_prefix
        payload_length += instructCache.system_prefix_length
    }

    if (instruct.system_prompt) {
        payload += `${instruct.system_prompt}`
    ...
    for (const message of messages.reverse()) {
        ...
        if (
            message_acc_length + payload_length + shard_length > maxLength &&
            !bypassContextLength
        ) {
            break
        }
    ...
    if (
        ...
        instruct.examples &&
        examples &&
        message_acc_length + payload_length + characterCache.examples_length < maxLength
    ) {
        payload += examples
    ...
    payload = replaceMacrosInternal(payload + message_acc)
...
const replaceMacrosInternal = (data: string) => {
    return replaceMacros(data, { extraMacros: thinkRule })
}
